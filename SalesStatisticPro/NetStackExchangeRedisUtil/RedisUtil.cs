...
namespace NetStackExchangeRedisUtil
{
    /// <summary>
    /// Redis操作
    /// </summary>
    public class RedisUtil
    {
        ...
        public RedisUtil(int dbNum = 0)
            : this(dbNum, null)
        {
        }

        public RedisUtil(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
        ...
        /// <param name="key">Redis Key</param>
        /// <returns></returns>
        public string StringGet1(string key)
        {
            key = AddSysCustomKey(key);
        ...
        /// <returns></returns>
        public string[] StringGet(List<string> listKey)
        {
            List<string> newKeys = listKey.Select(AddSysCustomKey).ToList();
            return Do(db => db.StringGet(ConvertRedisKeys(newKeys))).ToStringArray();
        }
        ...
        {
            key = AddSysCustomKey(key);
            List<RedisValue> dataKeys1 = new List<RedisValue>();
            return Do(db => db.HashDelete(key, dataKeys1.ToArray()));
        }
        ...
        public async Task<long> HashDeleteAsync(string key, List<string> dataKeys)
        {
            ...
            return await Do(db => db.HashDeleteAsync(key, dataKeys1.ToArray()));
        }
