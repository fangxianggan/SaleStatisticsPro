...
{
    public static class ZipFloHelper
    {
        #region 制作压缩包（单个文件压缩）  
        /// <summary>  
        /// 制作压缩包（单个文件压缩）  
        ...
        /// </summary>  
        /// <param name="topDirectoryName">多个文件的路径</param>  
        /// <param name="zipedFileName">压缩包文件名</param>  
        /// <param name="compresssionLevel">压缩级别 1-9</param>  
        /// <param name="password">密码</param>  
        /// <param name="comment">注释</param>  
        public static void ZipFiles(List<string> topDirectoryName, string zipedFileName, int compresssionLevel, string password, string comment)
        {
            using (ZipOutputStream zos = new ZipOutputStream(File.Open(zipedFileName, FileMode.OpenOrCreate)))
            {
                ...
                foreach (string file in topDirectoryName)
                {
                    if (File.Exists(file))
                    {
