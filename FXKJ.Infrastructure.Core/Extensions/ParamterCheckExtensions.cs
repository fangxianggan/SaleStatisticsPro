...
namespace FXKJ.Infrastructure.Core.Extensions
{
    ...
    public static class ParamterCheckExtensions
    {
        ...
        public static void CheckNotNullOrEmpty<T>(this IEnumerable<T> collection, string paramName)
        {
           ...
           // Require<ArgumentException>(collection.Any(), string.Format(ResourceSet.ParameterCheck_NotNullOrEmpty_Collection, paramName));
        }
        ...
        public static void CheckFileExists(this string filename, string paramName = null)
        {
         ...
         //   Require<FileNotFoundException>(File.Exists(filename), string.Format(ResourceSet.ParameterCheck_FileNotExists, filename));
        }
    }
}