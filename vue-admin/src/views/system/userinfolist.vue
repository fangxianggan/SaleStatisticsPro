<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-form :inline="true" :model="filterModel">
        <el-form-item label="手机号码">
          <el-input v-model="filterModel.phoneNumber.value" placeholder="手机号码" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        <el-form-item label="昵称">
          <el-input v-model="filterModel.nickName.value" placeholder="昵称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        <el-form-item label="用户名">
          <el-input v-model="filterModel.userName.value" placeholder="用户名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
    ...
    <el-table :key="tableKey"
      ...
      <el-table-column label="手机号" prop="phoneNumber" sortable="custom" width="180">
        <template slot-scope="{row}">
          <span>{{ row.phoneNumber }}</span>
        </template>
      </el-table-column>
      <el-table-column label="昵称" prop="nickName" sortable="custom" width="180">
        <template slot-scope="{row}">
          <span>{{ row.nickName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="用户名" prop="userName" sortable="custom" width="180">
        <template slot-scope="{row}">
          <span>{{ row.userName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="性别" prop="gender" sortable="custom" width="180">
        <template slot-scope="{row}">
          <span>{{ row.gender|formatGender }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="地址" prop="receAddress" sortable="custom" width="180px">
        <template slot-scope="{row}">
          <span>{{ row.receAddress}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="right" label-width="100px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="手机号" prop="phoneNumber" :rules="rules.checkNull">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="收货地址">
              <el-input v-model="temp.receAddress" type="textarea" :autosize="{minRows: 2,maxRows: 5}" />
            </el-form-item>
          </el-col>
        </el-row>
    ...
    methods: {
      ...
      handleDelete(row, index) {
        ...
        this.$confirm(title, '提示', {
          ...
          .then(() => {
            this.$ajax("/UserInfo/GetIsDeleteFlag", { phoneNumber: row.phoneNumber }, { method: "get" }).then(d => {
              if (d.resultSign == 1) {
