<template>
        ...
        <el-collapse-item title="查询条件" name="1">
          <el-form :inline="true" :model="filterModel">
            <el-form-item label="销售订单号">
              <el-input
                v-model="filterModel.sOrderNum.value"
                placeholder="销售订单号"
                class="filter-item"
            ...
            <el-form-item label="手机号">
              <el-input
                v-model="filterModel.phoneNumber.value"
                placeholder="用户手机号"
                class="filter-item"
                @keyup.enter.native="handleFilter"
              />
  ...
  <el-form-item label="产品名称">
            ...
            <el-form-item label="产品数量">
              <el-input
                v-model="filterModel.saleCount.value"
                placeholder="产品数量"
                class="input-with-select"
                @keyup.enter.native="handleFilter"
              >
                <el-select
                  v-model="filterModel.saleCount.method"
                  class="el-sel"
                  slot="prepend"
                  placeholder="请选择"
                >
            ...
            <el-form-item label="产品单价">
              <el-input
                v-model="filterModel.salePrice.value"
                placeholder="产品单价"
                class="input-with-select"
                @keyup.enter.native="handleFilter"
              >
                <el-select
                  v-model="filterModel.salePrice.method"
                  class="el-sel"
                  slot="prepend"
                  placeholder="请选择"
                >
    ...
    >
      ...
      <el-table-column type="expand">
        <template slot-scope="{row}">
          <el-table
            :data="row.saleOrderInfoViewModels"
            style="width: 100%"
          ...
          >
            ...
            <el-table-column prop="expressNumber" label="国内单号">
              <template slot-scope="{row}">
                <a
            ...
            <el-table-column prop="saleFreightAmount" label="国内运费"></el-table-column>
            <el-table-column prop="saleCount" label="购买数量"></el-table-column>
            <el-table-column prop="salePrice" label="单价"></el-table-column>
            <el-table-column prop="saleAmount" label="金额"></el-table-column>
            <el-table-column prop="saleFreightAmount" label="运费"></el-table-column>
            <el-table-column prop="saleSumAmount" label="总金额"></el-table-column>
          </el-table>
        </template>
      </el-table-column>
      <el-table-column
        ...
        prop="sOrderNum"
        sortable="custom"
        :class-name="getSortClass('sOrderNum')"
      >
        <template slot-scope="{row}">
    ...
    <el-dialog v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" width="100%">
      <el-form ref="dataForm" :model="temp" label-position="right" label-width="100px">
            ...
            <el-col :span="8">
              <el-form-item prop="phoneNumber" :rules="rules.checkNull" label="手机号码">
                <el-select
                  ...
                  clearable
                  placeholder="请选择手机号码"
                  :remote-method="getUserInfoList"
                  :loading="userInfoLoad"
                  @focus="getUserInfoList"
                >
                  <el-option
                    v-for="(item,index) in userInfoSListDetail"
                    :key="index"
        ...
        <fieldset>
          ...
          <el-table
            v-loading="listLoading"
            :data="temp.saleOrderInfoViewModels"
            border
            fit
            highlight-current-row
          ...
          >
            ...
            <el-table-column align="center" label="运费" width="120" prop="saleFreightAmount">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.saleFreightAmount'"
            ...
            <el-table-column align="center" label="商品名称" prop="sProductCode">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.sProductCode'"
                  :rules="rules.checkNull"
                >
                  <el-select
                    v-model="row.sProductCode"
                    remote
                    filterable
                    clearable
                    placeholder="请选择商品"
                    :remote-method="getProductStockDetail"
                    :loading="pProductCodeLoad"
                    @focus="getProductStockDetail"
                  >
                    <el-option
            ...
            <el-table-column align="center" label="数量" width="120" prop="saleCount">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.saleCount'"
                  :rules="rules.checkSaleCount"
                >
            ...
            <el-table-column align="center" label="单价" width="120" prop="salePrice">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.salePrice'"
            ...
            <el-table-column align="center" label="理赔金额" width="120" prop="saleSettlementAmount">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.saleSettlementAmount'"
                  :rules="rules.checkIntAndNull"
                >
            ...
            <el-table-column align="center" label="状态" width="120">
              <template slot-scope="{row,$index}">
                <el-form-item
                  :prop="'saleOrderInfoViewModels.'+$index+'.saleOrderInfoState'"
                  :rules="rules.checkNull"
                >
                  <el-select v-model="row.saleOrderInfoState">
                    <el-option
                      v-for="item in saleOrderInfoStateSList"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    ></el-option>
            ...
            <el-table-column align="center" label="操作" width="80">
              <template slot-scope="{row,$index}">
                <el-button
                  ...
                  size="mini"
                  type="success"
                  @click="handleAddSaleOrderInfo(row)"
                >新增</el-button>
                <el-button
                  v-if="$index!=0"
                  size="mini"
                  type="danger"
                  @click="handleDeleteSaleOrderInfo(row,$index)"
                >删除</el-button>
              </template>
            </el-table-column>
          </el-table>
...
export default {
      ...
      checkSaleCount: [
        ...
        },
        {
          validator: (rule, value, callback) => {
            ...
            //为空
            if (productCode == "") {
              myAction.getNotifyFunc(
                { message: "请选择商品", resultSign: 2 },
                this
              );
  ...
  methods: {
    getList() {
      ...
      let data = myAction.getQueryModel(
    ...
    sumSaleSumAmount(row) {
      ...
      if (saleSettlementAmount > 0) {
        ...
        allAmount =
          parseFloat(row.saleAmount) + parseFloat(row.saleFreightAmount);
      }
    ...
    setHJValue(prop, val) {
      switch (prop) {
        case "saleAmount":
          this.temp.allSaleAmount = val;
          break;
        case "saleFreightAmount":
          this.temp.allSaleFreightAmount = val;
          break;
        case "saleSettlementAmount":
          this.temp.allSaleSettlementAmount = val;
          break;
        case "saleSumAmount":
          this.temp.allSaleSumAmount = val;
          break;
        default:
          break;
      }
