<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-form :inline="true" :model="filterModel">
        <el-form-item label="订单号">
          <el-input v-model="filterModel.pOrderNum.value" placeholder="订单号" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        <el-form-item label="标题">
          <el-input v-model="filterModel.pOrderTitle.value" placeholder="标题" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        <el-form-item label="购买时间">
          <el-date-picker v-model="filterModel.pOrderCreateTime.value"
        ...
        <el-form-item label="美国单号">
          <el-input v-model="filterModel.usaNumber.value" placeholder="美国单号" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
        <el-form-item label="供应商">
          <el-select v-model="filterModel.businessCode.value" clearable filterable placeholder="供应商" @keyup.enter.native="handleFilter" @focus="getBusinessCodeSelectList">
        ...
        <el-form-item label="转运仓">
          <el-select v-model="filterModel.transferBinCode.value" clearable filterable placeholder="转运仓" @keyup.enter.native="handleFilter" @focus="getTransferBinCodeSelectList">
            <el-option v-for="(item,index) in transferBinCodeSList"
                       :key="index"
                       :label="item.label"
                       :value="item.value">
            </el-option>
    ...
    <el-table :key="tableKey"
      ...
      <el-table-column type="expand">
        <template slot-scope="{row}">
          <el-table :data="row.purchaseOrderInfoViewModels"
                    style="width: 100%"
            ...
            <el-table-column prop="expressFreightAmount"
                             label="国际运费">
            </el-table-column>
          
            <el-table-column prop="internationNumber"
                             label="国际转运单号">
              <template slot-scope="{row}">
                <a @click="queryLogisticsInfo(row.internationCompanyCode,row.internationNumber)">{{ row.internationNumber }}</a>
              </template>
            </el-table-column>
            <el-table-column prop="internationFreightAmount"
                             label="国际转运运费">
            </el-table-column>
           

            <el-table-column prop="domesticNumber"
                             label="国内转运单号">
              <template slot-scope="{row}">
                <a @click="queryLogisticsInfo(row.domesticCompanyCode,row.domesticNumber)">{{ row.domesticNumber }}</a>
              </template>
            </el-table-column>
            <el-table-column prop="domesticFreightAmount"
                             label="国内转运运费">
            </el-table-column>
            ...
            <el-table-column prop="purchaseSettlementAmount"
                             label="理赔金额">
            </el-table-column>
            <el-table-column prop="amount"
                             label="总金额">
            </el-table-column>
            <el-table-column prop="purchaseOrderInfoState"
                             label="状态">
              <template slot-scope="{row}">
      ...
      <el-table-column label="美国单号" prop="usaNumber" sortable="custom">
        <template slot-scope="{row}">
          <span>{{ row.usaNumber}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="220" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button v-if="row.purchaseOrderState!='FF02'" type="primary" size="mini" @click="handleUpdate(row)">
            修改
          </el-button>
          <el-button v-if="row.purchaseOrderState!='FF02'" size="mini" type="danger" @click="handleDelete(row,$index)">
            删除
          </el-button>
          <el-button v-if="row.purchaseOrderState!='FF02'" type="success" size="mini" @click="handleLock(row)">
            锁定
          </el-button>
    ...
    <el-dialog v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" fit width="100%">
      <el-form ref="dataForm" :model="temp" label-position="right" label-width="100px">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="进货时间" prop="pOrderCreateTime" :rules="rules.checkNull">
                <el-date-picker v-model="temp.pOrderCreateTime" typeof="date" value-format="yyyy-MM-dd" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="供应商" prop="businessCode" :rules="rules.checkNull">
                <el-select v-model="temp.businessCode" clearable filterable placeholder="请选择供应商" @focus="getBusinessCodeSelectList" @change="getBusinessCodeSelectChangeList">
                  <el-option v-for="(item,index) in businessCodeSList"
                             :key="index"
                             :label="item.label"
                             :value="item.value">
            ...
            <el-col :span="8">
              <el-form-item label="转运仓" prop="transferBinCode" :rules="rules.checkNull">
                <el-select v-model="temp.transferBinCode" clearable filterable placeholder="请选择转运仓" @focus="getTransferBinCodeSelectList" @change="getTransferBinCodeSelectChangeList">
                  <el-option v-for="(item,index) in transferBinCodeSList"
                             :key="index"
                             :label="item.label"
                             :value="item.value">
        ...
        <fieldset>
          ...
          <el-table v-loading="listLoading" :data="temp.purchaseOrderInfoViewModels" border fit highlight-current-row :summary-method="getSummaries" show-summary>
            ...
            <el-table-column align="center" label="国际运费(元)" width="60" prop="expressFreightAmount">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.expressFreightAmount'" :rules="rules.checkIntAndNull">
            ...
            <el-table-column align="center" label="国际转运快递">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.internationExpressCompanyCode'" >
            ...
            <el-table-column align="center" label="国际转运单号" width="180">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.internationNumber'">
                  ...
                  <a @click="queryLogisticsInfo(row.internationExpressCompanyCode,row.internationNumber)"><i class="el-icon-search"></i></a>
                </el-form-item>
              </template>
            </el-table-column>

            <el-table-column align="center" label="国际转运运费(元)" width="60" prop="internationFreightAmount">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.internationFreightAmount'" :rules="rules.checkIntAndNull">
            ...
            <el-table-column align="center" label="国内转运快递">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.domesticExpressCompanyCode'" >
            ...
            <el-table-column align="center" label="国内转运单号" width="180">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.domesticNumber'">
                  ...
                  <a @click="queryLogisticsInfo(row.domesticExpressCompanyCode,row.domesticNumber)"><i class="el-icon-search"></i></a>
                </el-form-item>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="国内运费(元)" width="60" prop="domesticFreightAmount">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.domesticFreightAmount'" :rules="rules.checkIntAndNull">
            ...
            <el-table-column align="center" label="商品名称">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.pProductCode'" :rules="rules.checkNull">
                  <el-select v-model="row.pProductCode" remote filterable clearable placeholder="请选择商品" :remote-method="getSelectProductDetial" :loading="pProductCodeLoad" @focus="getSelectProductDetial">
                    <el-option v-for="(item,index) in pProductCodeSListDetail"
                               :key="index"
                               :label="item.productName"
                               :value="item.productCode">
                      <span style="float: left">{{ item.productName }}</span>
                      <span style="float: right; color: #8492a6; font-size: 13px">{{ item.simpleCode }}</span>
                    ...
                    <el-form-item label="单位:">
                      <span>{{ item.unitName  }}</span>
                                      ...
                                      <span style="float: right; color: #8492a6; font-size: 13px" slot="reference">{{ item.simpleCode }}</span>
                                      </el-popover>-->
                    </el-option>
                  </el-select>
            ...
            <el-table-column align="center" label="数量" width="60" prop="purchaseCount">
              ...
              <template slot-scope="{row,$index}">
                ...
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.purchaseCount'" :rules="rules.checkIntAndNull">
            ...
            <el-table-column align="center" label="单价(元)" width="80" prop="purchasePrice">
              <template slot-scope="{row,$index}">
                ...
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.purchasePrice'" :rules="rules.validatePrice">
            ...
            <el-table-column align="center" label="理赔金额(元)" width="60" prop="purchaseSettlementAmount">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.purchaseSettlementAmount'" :rules="rules.checkIntAndNull">
            ...
            <el-table-column align="center" label="状态" width="80">
              <template slot-scope="{row,$index}">
                <el-form-item :prop="'purchaseOrderInfoViewModels.'+$index+'.purchaseOrderInfoState'" :rules="rules.checkNull">
                  <el-select v-model="row.purchaseOrderInfoState">
                    <el-option v-for="item in purchaseOrderInfoStateSList"
                               :key="item.value"
                               :label="item.label"
                               :value="item.value">
                    </el-option>
            ...
            <el-table-column align="center" label="操作" width="80">
              <template slot-scope="{row,$index}">
                <el-button v-if="$index==0" size="mini" type="success" @click="handleAddPurchaseOrderInfo(row)">
                  新增
                </el-button>
                <el-button v-if="$index!=0" size="mini" type="danger" @click="handleDeletePurchaseOrderInfo(row,$index)">
                  删除
                </el-button>
              </template>
            </el-table-column>
    ...
    <el-dialog title="物流信息"  :visible.sync="dialogLogisticsVisible" width="60%" top="50">
      <iframe :src="logisticsUrl" style="width:100%;height:1000px;overflow:hidden;">
      </iframe>
    </el-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      sumAmount(row) {
        ...
        if (settlementAmount > 0) {
          ...
          allAmount = parseFloat(row.purchaseAmount) + parseFloat(row.internationFreightAmount) + parseFloat(row.domesticFreightAmount) + parseFloat(row.expressFreightAmount)
        }
      ...
      setHJValue(prop, val) {
        switch (prop) {
          case "purchaseAmount": this.temp.allPurchaseAmount = val; break;
          case "internationFreightAmount": this.temp.allInternationFreightAmount = val; break;
          case "domesticFreightAmount": this.temp.allDomesticFreightAmount = val; break;
          case "purchaseSettlementAmount": this.temp.allPurchaseSettlementAmount = val; break;
          case "amount": this.temp.allAmount = val; break;
          default: break;
      ...
      handleUpdate(row) {
        ...
        if (row.purchaseOrderInfoViewModels == null) {
          fetchPurchaseOrderInfoList(row).then(response => {
            ...
            this.businessCodeSList = [{ label: this.temp.businessName, value: this.temp.businessCode }]
            this.transferBinCodeSList = [{ label: this.temp.transferBinName, value: this.temp.transferBinCode }]
          });
        } else {
          this.temp = Object.assign({}, row) // copy obj
          this.businessCodeSList = [{ label: this.temp.businessName, value: this.temp.businessCode }]
        ...
        this.dialogFormVisible = true
        if (this.pProductCodeSListDetail.length == 0) this.getSelectProductDetial("")
